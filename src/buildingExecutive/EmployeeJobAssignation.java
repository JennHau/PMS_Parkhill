...
public class EmployeeJobAssignation extends javax.swing.JFrame {
    ...
    public EmployeeJobAssignation(BuildingExecutive BE, String employeeID, String jobId, Complaint complaint, boolean complaintsPage) throws IOException {
        ...
        if (assignedJob != null) {
            this.complaint = (!assignedJob.getComplaintID().equals(BE.TF.empty)) ? new Complaint(assignedJob.getComplaintID()) : null;
        }
    ...
    private void formAdditionalDetailsSetUp() throws IOException {
        if (assignedJob != null) {
            ...
            String timeNeeded = assignedJob.getExpectedTimeRequired();
    ...
    private void assignedJobTableMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_assignedJobTableMouseClicked
        ...
        if (jobId != null) {
            try {
                setAssignedJob(jobId.toLowerCase());
    ...
    private AssignedJob getAllFields(int action) throws IOException {
        ...
        // job Id
        String fieldJobId = (this.assignedJob != null) ? this.assignedJob.getTaskID() : BE.getNewTaskId(BE.TF.employeeJobFile, 0);
    ...
    private ArrayList<String> checkJobClash(AssignedJob jobItems) throws IOException{
        ...
        if (dateInput != null) {
            ...
            for (AssignedJob eachJob : employeeJobList) {
                ...
                if (!jobItems.getTaskID().equals(jobId)) {
                    boolean ignore = false;
                    if (!jobItems.getJobID().equals("MS") && !jobItems.getJobID().equals("NS") ) {
